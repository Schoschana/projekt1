...
class DownloadsController: UITableViewController {
    ...
    @objc fileprivate func handleDownloadComplete(notification: Notification) {
        ...
        guard let index = self.episodes.index(where: { $0.title == episodeDownloadComplete.EpisodeTitle}) else {
            return
        }
    ...
    @objc fileprivate func handleDownloadProgress(notification: Notification) {
        ...
        guard let index = self.episodes.index(where: { $0.title == title }) else {
            return
        }
        guard let cell = tableView.cellForRow(at: IndexPath(row: index, section: 0 )) as? EpisodeCell else {
            return
        }
